...
const players = [
  {
    ...
    posisi: 'Bek',
    negara: ['Indonesia'],
    periods: [{ tahun_mulai: 1985, tahun_selesai: 1997 }],
  ...
  },
  {
    ...
    posisi: 'Winger',
    negara: ['Indonesia'],
    periods: [{ tahun_mulai: 2005, tahun_selesai: 2018 }],
...
async function main() {
  ...
  for (const player of players) {
    ...
    const created = await api('/items/players', {
      method: 'POST',
      body: JSON.stringify({
        slug: player.slug,
        nama: player.nama,
    ...
    for (const period of player.periods) {
      await api('/items/player_periods', {
        method: 'POST',
        body: JSON.stringify({ player: created.data.id, ...period })
      })
    }
